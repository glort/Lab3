...
public class Hand {
	...
	public Hand()
	{
		for (Card card: CardsInHand){
			if (card.getWild() | card.getRank() == eRank.JOKER){
				HasJoker = true;
			}
		}
	}
	...
	private static ArrayList<Hand> HandleJoker(Hand hand){
		ArrayList<Hand> PossibleHands = new ArrayList<Hand>();
		ArrayList<Integer> WildsJokers = new ArrayList<Integer>();
		int counter1 = 0;
		int counter2 = 0;
		for (Card card: hand.CardsInHand){
			if (card.getRank() == eRank.JOKER|card.getWild() == true){
				WildsJokers.add(counter2);
				counter1++; 
			}
			counter2++;
		}
		...
		if (counter1 == 0)
			...
			for (short i = 0; i <= 3; i++) {
				...
				for (short j = 0; j <= 12; j++) {
					...
					hand.CardsInHand.set(WildsJokers.get(0), NewCard );
					PossibleHands.add(hand);
				}
			}
			...
			for (short i = 0; i <= 3; i++) {
				...
				for (short j = 0; j <= 12; j++) {
					...
					for (short k = 0; k <= 3; k++) {
						SuitValue = eSuit.values()[k];			
						for (short l = 0; l <= 12; l++) {
							RankValue = eRank.values()[l];				
							NewCard = new Card(SuitValue,RankValue, (13 * k) + l+1);
							hand.CardsInHand.set(WildsJokers.get(1), NewCard );
							PossibleHands.add(hand);
						}
					}
				...
				for (short j = 0; j <= 12; j++) {
					...
					for (short k = 0; k <= 3; k++) {
						...
						for (short l = 0; l <= 12; l++) {
							...
							hand.CardsInHand.set(WildsJokers.get(1), NewCard );
							for (short m = 0; m <= 3; m++) {
								SuitValue = eSuit.values()[m];			
								for (short n = 0; n <= 12; n++) {
									RankValue = eRank.values()[n];				
									NewCard = new Card(SuitValue,RankValue, (13 * m) + n+1);
									hand.CardsInHand.set(WildsJokers.get(2), NewCard );
									PossibleHands.add(hand);
								}
							}
						}
						...
						for (short l = 0; l <= 12; l++) {
							...
							for (short m = 0; m <= 3; m++) {
								...
								for (short n = 0; n <= 12; n++) {
									...
									hand.CardsInHand.set(WildsJokers.get(2), NewCard );
									for (short o = 0; o <= 3; o++) {
										SuitValue = eSuit.values()[o];			
										for (short p = 0; p <= 12; p++) {
											RankValue = eRank.values()[p];				
											NewCard = new Card(SuitValue,RankValue, (13 * o) + p+1);
											hand.CardsInHand.set(WildsJokers.get(3), NewCard );
											PossibleHands.add(hand);
										}
									}
								}
								...
								for (short n = 0; n <= 12; n++) {
									...
									for (short o = 0; o <= 3; o++) {
										...
										for (short p = 0; p <= 12; p++) {
											...
											hand.CardsInHand.set(WildsJokers.get(3), NewCard );
											for (short q = 0; q <= 3; q++) {
												SuitValue = eSuit.values()[q];			
												for (short r = 0; r <= 12; r++) {
													RankValue = eRank.values()[r];				
													NewCard = new Card(SuitValue,RankValue, (13 * q) + r+1);
													hand.CardsInHand.set(WildsJokers.get(4), NewCard );
													PossibleHands.add(hand);
												}
											}
										}
